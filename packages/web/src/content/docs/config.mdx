...
Use `experimental.subagent_max_depth` to control how deep subagents can [nest](/docs/agents#nesting). The root session is depth 1; values are clamped to 1–10.
...
The default is `5`. Setting it to `2` restores the previous spawn behavior (the main session spawns subagents, subagents do not); additionally, the `workflow` tool is removed at the limit (a security fix), and custom agents that previously nested without limit are now bounded. Setting it to `1` is the kill switch: the `task` and `workflow` tools are removed everywhere, including from the main session.

#### Subagent tree limit

Use `experimental.subagent_tree_limit` to control how many subagents a single session tree may start over its lifetime. The cap is an in-memory, per-process safety ceiling against runaway delegation; values are clamped to 1–10000.
...
The default is `200`. Resumed subagents and workflow dispatches do not count against it, and the counter is not persisted — it resets on each OpenCode process restart. Reaching the limit fails further spawns with a typed `SubagentTreeLimitError`.
