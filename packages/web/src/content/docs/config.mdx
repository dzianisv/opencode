...
OpenCode tracks workspace snapshots (via an internal Git store) to power step review and revert behavior. You can disable this with the `snapshot` option.
...
- Step snapshot diffs are not generated.
- Revert flows that depend on snapshots are limited.
- Session history still works, but without snapshot-based file state restore.

:::note
This is different from memory profiling snapshots created by `opencode debug memory --snapshot`.
:::
