...
The default is `200`. This is an in-memory safety ceiling counted per OpenCode process: resumed subagents and workflow dispatches do not count against it, and the counter resets on restart. Values are clamped to the range 1–10000; reaching the cap fails further spawns with `SubagentTreeLimitError`.
