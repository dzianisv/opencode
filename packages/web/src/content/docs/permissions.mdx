...
When the request comes from a [nested subagent](/docs/agents#nesting), the prompt is attributed to the **root session** of the session tree — you answer it there, without navigating into child sessions. The prompt's `metadata.originSessionID`, `metadata.originAgent`, and `metadata.originDepth` identify the subagent that asked. Answering `always` sticks to that root session and applies tree-wide for the rest of the OpenCode process — it covers every session in the same tree (at any depth), but it does not carry over to other session trees in the same instance.
